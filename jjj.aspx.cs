...
public partial class jjj : System.Web.UI.Page
{
    ...
    private void FillDetails()
    {
        ...
        qry = "SELECT *,CASE WHEN status=1 THEN 'Enable' ELSE 'disable' END AS status1 FROM Category_tb where name LIKE '" + txtSearch.Text + "%'";
        dt = c1.GetDT(qry);
   ...
   protected void DataList1_ItemCommand1(object source, RepeaterCommandEventArgs e)
    {
        ...
        if (e.CommandName == "change_status")
        {
            if (status.Equals("0"))
            {
                qry1 = "Update Category_tb set status = 1 where  CAid=" + id;
                c1.ExeQry(qry1);
                FillDetails();
            }
            else
            {
                qry1 = "Update Category_tb set status = 0 where  CAid=" + id;
                c1.ExeQry(qry1);
                FillDetails();
            }
        }
        ...
        {
            Session.Add("Upid", id);
            Response.Redirect("Category_update.aspx");
        }
   ...
   protected void btnSearch_Click(object sender, EventArgs e)
   {
    ...
    public static List<string> GetSuggestions(string prefix)
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("SELECT DISTINCT name FROM Category_tb WHERE name LIKE @Prefix + '%'", con))
            {
